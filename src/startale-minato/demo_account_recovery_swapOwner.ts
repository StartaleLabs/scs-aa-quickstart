...
const main = async () => {
    ...
    try {
      ...
      const updateOwnerData = encodeFunctionData({
      ...
      const calls = [
        {
          to: ecdsaValidatorAddress as Address,
          target: ecdsaValidatorAddress as Address,
          value: BigInt(0),
          data: updateOwnerData,
          callData: updateOwnerData,
        },
      ];
      console.log("Calls to update owner in the default ECDSA validator: ", calls);
      ...
      const ownerBefore = (await publicClient.readContract({
        address: ecdsaValidatorAddress as Address,
        abi: ECDSAValidatorAbi,
    ...
    const isOwner = ownerAfter === guardian2.address;
    console.log("Is guardian2 an owner? (has been added through recovery)", isOwner);

    const isOriginalEOAOwner = ownerAfter === eoaAddress;
    console.log("Is original EOA an owner? (was already an owner)", isOriginalEOAOwner);
